class RoomChannel < ApplicationCable::Channel
  ...
  def unsubscribed
    ...
    # Log no audit se o usuário (não Contact) ainda estiver offline após PRESENCE_DURATION
    # (ou seja, não reconectou dentro da janela)
    if @current_user.is_a?(User)
      Starchat::LogUserOfflineJob
        .set(wait: ::OnlineStatusTracker::PRESENCE_DURATION + 5.seconds)
        .perform_later(@current_user.id, @current_account.id, 'connection_lost')
    end
  end
