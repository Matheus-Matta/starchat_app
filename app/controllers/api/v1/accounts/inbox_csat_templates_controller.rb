class Api::V1::Accounts::InboxCsatTemplatesController < Api::V1::Accounts::BaseController
  ...
  before_action :validate_cosmos_enabled, only: [:analyze]
  ...
  def validate_cosmos_enabled
    return if Current.account.feature_enabled?('cosmos_integration')

    render json: { error: 'Cosmos is required for template analysis' }, status: :forbidden
  end
