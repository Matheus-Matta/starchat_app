...
const updateTool = toolDetails =>
  store.dispatch('cosmosCustomTools/update', {
    id: props.selectedTool.id,
    ...toolDetails,
  });

const i18nKey = computed(
  () => `COSMOS.CUSTOM_TOOLS.${props.type.toUpperCase()}`
);

const createTool = toolDetails =>
  store.dispatch('cosmosCustomTools/create', toolDetails);
...
<template>
  <Dialog
    ...
    :description="$t('COSMOS.CUSTOM_TOOLS.FORM_DESCRIPTION')"
