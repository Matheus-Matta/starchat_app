...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <div
        v-if="isLoading"
        class="mt-4 grid auto-rows-[16rem] gap-4 grid-cols-1 sm:grid-cols-[repeat(auto-fit,minmax(18rem,22rem))] sm:justify-start"
      >
        <div
          v-for="index in 3"
          :key="`agent-skeleton-${index}`"
          class="min-h-[16rem] rounded-xl border border-dashed border-n-weak bg-n-alpha-3 animate-pulse"
        />
      </div>
