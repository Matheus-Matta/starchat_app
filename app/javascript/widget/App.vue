...
export default {
  ...
  mounted() {
    const { websiteToken, locale, widgetColor } = window.starchatsWebChannel;
  ...
  methods: {
    ...
    setLocale(localeWithVariation) {
      ...
      const { enabledLanguages } = window.starchatsWebChannel;
    ...
    registerCampaignEvents() {
      ...
      emitter.on('execute-campaign', campaignDetails => {
        ...
        const { websiteToken } = window.starchatsWebChannel;
