class ReportingEventListener < BaseListener
  ...
  def first_reply_created(event)
    ...
    last_activity_at = last_non_human_activity(conversation, message.created_at)
    first_response_time = message.created_at.to_i - last_activity_at.to_i

    reporting_event = ReportingEvent.new(
      name: 'first_response',
      value: first_response_time,
      value_in_business_hours: business_hours(conversation.inbox, last_activity_at, message.created_at),
      account_id: conversation.account_id,
      inbox_id: conversation.inbox_id,
      user_id: message.sender_id,
      conversation_id: conversation.id,
      event_start_time: last_activity_at,
  ...
  def conversation_cosmos_inference_resolved(event)
    create_cosmos_inference_event(event, 'conversation_cosmos_inference_resolved')
  end

  def conversation_cosmos_inference_handoff(event)
    create_cosmos_inference_event(event, 'conversation_cosmos_inference_handoff')
  end
  ...
  def create_cosmos_inference_event(event, event_name)
    conversation = extract_conversation_and_account(event)[0]
  ...
  def safe_rollup(reporting_event)
    ...
    StarchatsExceptionTracker.new(e, account: reporting_event.account).capture_exception
  end
end
