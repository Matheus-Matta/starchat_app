...
#  idx_data_import_mappings_on_record              (starchats_record_type,starchats_record_id)
...
class DataImportMapping < ApplicationRecord
  ...
  validates :source_provider, :source_object_type, :source_object_id, :starchats_record_type, :starchats_record_id, presence: true
  validates :source_object_id, uniqueness: { scope: [:account_id, :source_provider, :source_object_type] }

  def starchats_record
    starchats_record_type.constantize.find_by(id: starchats_record_id)
  end
end
