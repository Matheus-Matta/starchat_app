class Notification::PushTestService
  ...
  def self.default_title
    format(DEFAULT_TITLE, installation_name: GlobalConfigService.load('INSTALLATION_NAME', 'Starchats'))
  end
  ...
  def test_fcm(subscription)
    return result(subscription, 'fcm', :skipped, 'Firebase credentials not configured') unless firebase_credentials_present?
