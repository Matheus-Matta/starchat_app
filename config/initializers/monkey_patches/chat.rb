      ...
      module Endpoints
        module Chat
          ...
          def chat_unfurl(options = {})
            raise ArgumentError, 'Either a combination of :channel and :ts or :unfurl_id and :source is required' if (options[:channel].nil? || options[:ts].nil?) && (options[:unfurl_id].nil? || options[:source].nil?)
