...
# Description: Install and manage a Starchats installation.
...
STARCHATS_HUB_URL="https://hub.2.starchats.com/events"
...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/starchats-setup.log' for details.\n"
   exit 1
  fi
}
...
function get_domain_info() {
  read -rp 'Enter the domain/subdomain for Starchats (e.g., starchats.domain.com): ' domain_name
...
serve Starchats at https://$domain_name.
...
function create_cw_user() {
  if ! id -u "starchats"; then
    adduser --disabled-password --gecos "" starchats
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/starchats/config
  file="/opt/starchats/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/starchats/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create starchats db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER starchats CREATEDB;
    ALTER USER starchats PASSWORD :'pass';
    ALTER ROLE starchats SUPERUSER;
...
function setup_starchats() {
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=starchats/' .env
...
function run_db_migrations(){
  sudo -i -u starchats << EOF
  cd starchats
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:starchats_prepare
EOF
}

##############################################################################
# Setup Starchats systemd services and cwctl CLI
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function configure_systemd_services() {
  ...
  if [ -f "/etc/systemd/system/starchats.target" ]; then
  ...
  if [ "$DEPLOYMENT_TYPE" == "web" ]; then
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop starchats.target || true
      systemctl disable starchats.target || true
      systemctl stop starchats-worker.1.service || true
      systemctl disable starchats-worker.1.service || true
    fi

    # Stop and disable worker target if converting from worker-only
    if [ -f "/etc/systemd/system/starchats-worker.target" ]; then
      echo "Converting from worker-only to web-only"
      systemctl stop starchats-worker.target || true
      systemctl disable starchats-worker.target || true
    fi

    cp /home/starchats/starchats/deployment/starchats-web.1.service /etc/systemd/system/starchats-web.1.service
    cp /home/starchats/starchats/deployment/starchats-web.target /etc/systemd/system/starchats-web.target

    systemctl daemon-reload
    systemctl enable starchats-web.target
    systemctl start starchats-web.target
    ...
    if [ "$existing_full_deployment" = true ]; then
      ...
      systemctl stop starchats.target || true
      systemctl disable starchats.target || true
      systemctl stop starchats-web.1.service || true
      systemctl disable starchats-web.1.service || true
    fi

    # Stop and disable web target if converting from web-only
    if [ -f "/etc/systemd/system/starchats-web.target" ]; then
      echo "Converting from web-only to worker-only"
      systemctl stop starchats-web.target || true
      systemctl disable starchats-web.target || true
    fi

    cp /home/starchats/starchats/deployment/starchats-worker.1.service /etc/systemd/system/starchats-worker.1.service
    cp /home/starchats/starchats/deployment/starchats-worker.target /etc/systemd/system/starchats-worker.target

    systemctl daemon-reload
    systemctl enable starchats-worker.target
    systemctl start starchats-worker.target
    ...
    cp /home/starchats/starchats/deployment/starchats-web.1.service /etc/systemd/system/starchats-web.1.service
    cp /home/starchats/starchats/deployment/starchats-worker.1.service /etc/systemd/system/starchats-worker.1.service
    cp /home/starchats/starchats/deployment/starchats.target /etc/systemd/system/starchats.target

    systemctl daemon-reload
    systemctl enable starchats.target
    systemctl start starchats.target
  fi

  cp /home/starchats/starchats/deployment/starchats /etc/sudoers.d/starchats
  cp /home/starchats/starchats/deployment/setup_20.04.sh /usr/local/bin/cwctl
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/starchats/starchats/develop/deployment/nginx_starchats.conf
  cp nginx_starchats.conf /etc/nginx/sites-available/nginx_starchats.conf
  certbot certonly --non-interactive --agree-tos --nginx -m "$le_email" -d "$domain_name"
  sed -i "s/starchats.domain.com/$domain_name/g" /etc/nginx/sites-available/nginx_starchats.conf
  ln -s /etc/nginx/sites-available/nginx_starchats.conf /etc/nginx/sites-enabled/nginx_starchats.conf
...
EOF
  ...
  # Restart the appropriate starchats target
  if [ -f "/etc/systemd/system/starchats-web.target" ]; then
    systemctl restart starchats-web.target
  elif [ -f "/etc/systemd/system/starchats-worker.target" ]; then
    systemctl restart starchats-worker.target
  else
    systemctl restart starchats.target
  fi
}
...
function setup_logging() {
  touch /var/log/starchats-setup.log
  LOG_FILE="/var/log/starchats-setup.log"
}
...
***************************************************************************
Woot! Woot!! Starchats server installation is complete.
The server will be accessible at https://$domain_name

Join the community at https://starchats.com/community?utm_source=cwctl
***************************************************************************
...
function cwctl_message() {
  echo $'\U0001F680 Try out the all new Starchats CLI tool to manage your installation.'
...
function get_cw_version() {
  CW_VERSION=$(curl -s https://app.starchats.com/api | python3 -c 'import sys,json;data=json.loads(sys.stdin.read()); print(data["version"])')
}
...
***************************************************************************
              Starchats Installation (v$CW_VERSION)
***************************************************************************

For more verbose logs, open up a second terminal and follow along using,
'tail -f /var/log/starchats-setup.log'.

EOF

  sleep 3
  read -rp 'Would you like to configure a domain and SSL for Starchats?(yes or no): ' configure_webserver
  ...
  echo "➥ 6/9 Installing Starchats. This takes a long while."
  setup_starchats &>> "${LOG_FILE}"
...
https://www.starchats.com/docs/deployment/deploy-starchats-in-linux-vm?utm_source=cwctl
...
function get_console() {
  sudo -i -u starchats bash -c " cd starchats && RAILS_ENV=production bundle exec rails c"
}
...
function help() {
  ...
  cat <<EOF
Usage: cwctl [OPTION]...
Install and manage your Starchats installation.
...
Installation/Upgrade:
  -i, --install             Install the latest stable version of Starchats
  -I BRANCH                 Install Starchats from a git branch
  -u, --upgrade             Upgrade Starchats to the latest stable version
  -U BRANCH                 Upgrade Starchats from a git branch (EXPERIMENTAL)
...
Management:
  ...
  -l, --logs                View logs from Starchats. Supported values include web/worker.
  -r, --restart             Restart Starchats server
...
Get help, https://starchats.com/community?utm_source=cwctl

EOF
}

##############################################################################
# Get Starchats web/worker logs (-l/--logs)
# Globals:
#   None
# Arguments:
#   None
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u starchats-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u starchats-web.1.service -f
  fi
}
...
function upgrade_redis() {
  ...
  echo "Upgrading Redis to v7+ for Rails 7 support(Starchats v2.17+)"
...
##############################################################################
# Install pnpm - this replaces yarn starting from Starchats 4.0
# Globals:
#   None
# Arguments:
#   None
...
function upgrade() {
  cwctl_upgrade_check
  get_cw_version
  echo "Upgrading Starchats to v$CW_VERSION (branch: $BRANCH)"
...
EOF
  ...
  if [[ "$(printf '%s\n' "$CW_VERSION" "4.0" | sort -V | head -n 1)" == "4.0" ]]; then
    echo "Starchats v4.0 and above requires pgvector support in PostgreSQL."
  ...
  # Navigate to the Starchats directory
  cd starchats
...
##############################################################################
# Restart Starchats server (-r/--restart)
# Globals:
#   None
# Arguments:
#   None
...
function restart() {
  if [ -f "/etc/systemd/system/starchats-web.target" ]; then
    systemctl restart starchats-web.target
    systemctl status starchats-web.target
  elif [ -f "/etc/systemd/system/starchats-worker.target" ]; then
    systemctl restart starchats-worker.target
    systemctl status starchats-worker.target
  else
    systemctl restart starchats.target
    systemctl status starchats.target
  fi
}

##############################################################################
# Convert existing Starchats deployment to different type (--convert)
# Globals:
#   DEPLOYMENT_TYPE
# Arguments:
#   None
...
function convert_deployment() {
  echo "Converting Starchats deployment to: $DEPLOYMENT_TYPE"

  # Check if Starchats is installed
  if [ ! -d "/home/starchats/starchats" ]; then
    echo "Starchats installation not found. Use --install first."
    exit 1
  fi
...
function report_event() {
  ...
  curl -X POST -H "Content-Type: application/json" -d "$data" "$STARCHATS_HUB_URL" -s -o /dev/null
}
...
function get_installation_identifier() {
  ...
  installation_identifier=$(sudo -i -u starchats << "EOF"
  cd starchats
...
function cwctl_upgrade_check() {
    ...
    local remote_version_url="https://raw.githubusercontent.com/starchats/starchats/master/VERSION_CWCTL"
...
function upgrade_cwctl() {
    wget https://get.starchats.app/linux/install.sh -O /usr/local/bin/cwctl > /dev/null 2>&1 && chmod +x /usr/local/bin/cwctl
}
