class Integrations::Cosmos::ProcessorService < Integrations::BotProcessorService
  pattr_initialize [:event_name!, :hook!, :event_data!]

  private

  def get_response(_session_id, message_content)
    call_cosmos(message_content)
  end
  ...
  def call_cosmos(message_content)
    url = "#{GlobalConfigService.load('COSMOS_API_URL',
