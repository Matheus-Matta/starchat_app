...
# Seeds Cosmos assistant activity for the reports/overview test data.
#
# Produces a variety of assistant-handled conversations in a single web inbox so
# every Cosmos assistant overview metric (handled, auto-resolution, handoff,
# hours saved, reopen rate, conversation depth) has realistic data:
#   - :resolved_by_assistant  assistant answers and Cosmos auto-resolves
...
#   - :resolved_and_reopened  Cosmos resolves, then the conversation reopens
#
...
class Seeders::Reports::AssistantConversationCreator
  ...
  def apply_outcome(conversation, created_at, outcome)
    ...
    when :resolved_by_assistant
      resolve_by_cosmos(conversation, resolved_at)
  ...
  def resolve_by_cosmos(conversation, resolved_at)
    mark_resolved(conversation, resolved_at)
    travel_to(resolved_at) do
      trigger_event('conversation_resolved', conversation)
      trigger_event('conversation_cosmos_inference_resolved', conversation)
    end
    travel_back
  end
  ...
  def handoff_to_human(conversation)
    trigger_event('conversation_cosmos_inference_handoff', conversation)
  end
