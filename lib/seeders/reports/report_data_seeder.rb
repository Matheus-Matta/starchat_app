...
#   - 1 Cosmos assistant bound to a single web inbox, with knowledge (FAQs + documents)
...
class Seeders::Reports::ReportDataSeeder
  ...
  # Cosmos assistant conversations, split across the outcomes the overview page reports on.
  ...
  # Delete Cosmos records directly (assistant associations are destroy_async, which
  # would leave rows around mid-reseed); order respects foreign keys.
  def clear_assistant_data
    assistant_ids = Cosmos::Assistant.for_account(@account.id).select(:id)
    Cosmos::AssistantResponse.by_account(@account.id).delete_all
    Cosmos::Document.for_account(@account.id).delete_all
    CosmosInbox.where(cosmos_assistant_id: assistant_ids).delete_all
    Cosmos::Assistant.for_account(@account.id).delete_all
  end
  ...
  def create_assistant
    @account.enable_features!('cosmos_integration', 'cosmos_integration_v2')
    @assistant_inbox = @inboxes.first
    @assistant = Cosmos::Assistant.create!(
      account: @account,
      name: "#{Faker::Company.name} Copilot",
      description: 'Cosmos assistant handling website support conversations.',
      config: { feature_faq: true, feature_memory: true, product_name: @account.name }
    )
    CosmosInbox.create!(cosmos_assistant: @assistant, inbox: @assistant_inbox)
  ...
  def create_assistant_response(status)
    Cosmos::AssistantResponse.create!(
      account: @account,
      assistant: @assistant,
