...
namespace :cosmos do
  desc 'Start interactive chat with Cosmos assistant - Usage: rake cosmos:chat[assistant_id] or rake cosmos:chat -- assistant_id'
  task :chat, [:assistant_id] => :environment do |_, args|
    assistant_id = args[:assistant_id] || ARGV[1]

    unless assistant_id
      puts '❌ Please provide an assistant ID'
      puts 'Usage: rake cosmos:chat[assistant_id]'
      puts "\nAvailable assistants:"
      Cosmos::Assistant.includes(:account).each do |assistant|
        puts "  ID: #{assistant.id} - #{assistant.name} (Account: #{assistant.account.name})"
      end
      exit 1
    end

    assistant = Cosmos::Assistant.find_by(id: assistant_id)
    unless assistant
    ...
    chat_session = CosmosChatSession.new(assistant)
    chat_session.start
  end
end
...
class CosmosChatSession
  ...
  def generate_assistant_response
    runner = Cosmos::Assistant::AgentRunnerService.new(assistant: @assistant, callbacks: build_callbacks)
