...
describe ChatwootHub do
  ...
  context 'when fetching cosmos settings' do
    it 'returns the cosmos settings' do
      account = create(:account)
      stub_request(:post, ChatwootHub::COSMOS_ACCOUNTS_URL).with(
      ...
      expect(described_class.get_cosmos_settings(account).body).to eq(
        {
