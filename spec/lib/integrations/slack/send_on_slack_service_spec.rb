  ...
  describe '#perform' do
    ...
    context 'with identifier' do
      ...
      it 'sent lnk unfurl to slack' do
        unflur_payload = { :channel => 'channel',
                           :ts => 'timestamp',
                           :unfurls =>
           { :'https://qa.starchats.com.br/app/accounts/1/conversations/1' =>
             { :blocks => [{ :type => 'section',
    ...
    context 'when message contains mentions' do
      it 'sends formatted message to slack along with inbox name when identifier not present' do
        inbox = conversation.inbox
        message.update!(content: "Hi [@#{contact.name}](mention://user/#{contact.id}/#{contact.name}), welcome to Starchats!")
