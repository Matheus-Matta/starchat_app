...
RSpec.describe Llm::FeatureRouter do
  ...
  describe '.resolve' do
    ...
    it 'resolves GPT-5.2 as the assistant default when Cosmos V2 is enabled without storing an account override' do
    ...
    it 'keeps account model overrides ahead of the Cosmos V2 default' do
      account.enable_features!('cosmos_integration_v2')
      account.update!(cosmos_models: { 'assistant' => 'gpt-5.1' })
    ...
    it 'falls back to the feature default when the account override is blank' do
      account.update!(cosmos_models: { 'editor' => '' })
