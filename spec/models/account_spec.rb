...
RSpec.describe Account do
  ...
  describe 'usage_limits' do
    ...
    it 'returns StarchatsApp.max limits' do
      expect(account.usage_limits[:agents]).to eq(StarchatsApp.max_limit)
      expect(account.usage_limits[:inboxes]).to eq(StarchatsApp.max_limit)
    end
  end
  ...
  describe 'cosmos defaults for new accounts' do
    it 'does not store Cosmos model overrides or enable premium Cosmos features' do
  ...
  describe 'support_email' do
    ...
    it 'returns the support email from inbox if inbox value is present' do
      account.update(support_email: 'support@starchats.com')
      with_modified_env MAILER_SENDER_EMAIL: 'hello@starchats.com' do
        expect(account.support_email).to eq('support@starchats.com')
      end
    end
    ...
    it 'returns the support email from ENV if inbox value is nil' do
      ...
      with_modified_env MAILER_SENDER_EMAIL: 'hello@starchats.com' do
        expect(account.support_email).to eq('hello@starchats.com')
      end
    end
  ...
  describe 'cosmos_preferences' do
    ...
    describe 'with no saved preferences' do
      ...
      it 'returns GPT-5.2 for assistant when Cosmos V2 is enabled' do
    ...
    describe 'validation' do
      ...
      it 'rejects unknown feature keys' do
        account.cosmos_models = { 'unknown_feature' => 'gpt-4.1' }

        expect(account).not_to be_valid
        expect(account.errors[:cosmos_models]).to include("'unknown_feature' is not a known feature")
      end

      it 'removes blank model overrides before saving' do
        account.update!(cosmos_models: { 'editor' => '', 'assistant' => 'gpt-5.2' })

        expect(account.cosmos_models).to eq('assistant' => 'gpt-5.2')
      end
    end
  end
end
