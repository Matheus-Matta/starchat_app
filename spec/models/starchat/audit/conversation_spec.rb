...
RSpec.describe 'Conversation Audit', type: :model do
  ...
  before do
    ...
    conversation.class.send(:include, Starchat::Audit::Conversation) if defined?(Starchat::Audit::Conversation)
  end

  describe 'audit logging on destroy' do
    it 'creates an audit log when conversation is destroyed' do
      skip 'Starchat audit module not available' unless defined?(Starchat::Audit::Conversation)
