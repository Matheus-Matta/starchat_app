...
RSpec.describe MessageContentPresenter do
  ...
  describe '#outgoing_content' do
    ...
    context 'when inbox is a non-rendering channel (e.g. WhatsApp)' do
      let(:conversation) do
        create(:conversation, inbox: create(:channel_whatsapp, sync_templates: false, validate_provider_config: false).inbox)
      end
      let(:content_type) { 'text' }
      let(:content) { "First line\\\nSecond line" }

      it 'strips the markdown hard-break backslash instead of sending it literally' do
        expect(presenter.outgoing_content).to eq("First line\nSecond line")
      end
    end
    ...
    context 'when message is input_csat and inbox is not web widget' do
      ...
      it 'returns I18n default message when no CSAT config and dynamically generates survey URL (HTML format)' do
        with_modified_env 'FRONTEND_URL' => 'https://app.starchats.com.br' do
          expected_url = "https://app.starchats.com.br/survey/responses/#{conversation.uuid}"
