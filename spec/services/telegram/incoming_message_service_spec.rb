...
describe Telegram::IncomingMessageService do
  before do
    ...
    stub_request(:get, 'https://starchats-assets.local/sample.png').to_return(
      status: 200,
      body: File.read('spec/assets/sample.png'),
      headers: {}
    )
    stub_request(:get, 'https://starchats-assets.local/sample.mov').to_return(
      status: 200,
      body: File.read('spec/assets/sample.mov'),
      headers: {}
    )
    stub_request(:get, 'https://starchats-assets.local/sample.mp3').to_return(
      status: 200,
      body: File.read('spec/assets/sample.mp3'),
      headers: {}
    )
    stub_request(:get, 'https://starchats-assets.local/sample.ogg').to_return(
      status: 200,
      body: File.read('spec/assets/sample.ogg'),
      headers: {}
    )
    stub_request(:get, 'https://starchats-assets.local/sample.pdf').to_return(
      status: 200,
  ...
  describe '#perform' do
    context 'when valid text message params' do
      it 'creates appropriate conversations, message and contacts' do
        ...
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.name).to eq('Sojan Jose')
    ...
    context 'when valid caption params' do
      it 'creates appropriate conversations, message and contacts' do
        ...
        described_class.new(inbox: telegram_channel.inbox, params: params).perform
        expect(telegram_channel.inbox.conversations.count).not_to eq(0)
        contact = Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first
    ...
    context 'when valid audio messages params' do
      it 'creates appropriate conversations, message and contacts' do
        allow(telegram_channel.inbox.channel).to receive(:get_telegram_file_path).and_return('https://starchats-assets.local/sample.mp3')
        params = {
          'update_id' => 2_342_342_343_242,
          'message' => {
            'audio' => {
        ...
        expect(telegram_channel.inbox.conversations.count).not_to eq(0)
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.name).to eq('Sojan Jose')
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.additional_attributes['social_telegram_user_id']).to eq(23)
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.additional_attributes['social_telegram_user_name']).to eq('sojan')
    ...
    context 'when valid image attachment params' do
      it 'creates appropriate conversations, message and contacts' do
        allow(telegram_channel.inbox.channel).to receive(:get_telegram_file_path).and_return('https://starchats-assets.local/sample.png')
        params = {
          'update_id' => 2_342_342_343_242,
          'message' => {
            'photo' => [{
    ...
    context 'when valid video messages params' do
      it 'creates appropriate conversations, message and contacts' do
        allow(telegram_channel.inbox.channel).to receive(:get_telegram_file_path).and_return('https://starchats-assets.local/sample.mov')
        params = {
          'update_id' => 2_342_342_343_242,
          'message' => {
            'video' => {
    ...
    context 'when valid voice attachment params' do
      it 'creates appropriate conversations, message and contacts' do
        allow(telegram_channel.inbox.channel).to receive(:get_telegram_file_path).and_return('https://starchats-assets.local/sample.ogg')
        params = {
          'update_id' => 2_342_342_343_242,
          'message' => {
            'voice' => {
    ...
    context 'when valid document message params' do
      it 'creates appropriate conversations, message and contacts' do
        allow(telegram_channel.inbox.channel).to receive(:get_telegram_file_path).and_return('https://starchats-assets.local/sample.pdf')
        params = {
          'update_id' => 2_342_342_343_242,
          'message' => {
            'document' => {
    ...
    context 'when valid callback_query params' do
      it 'creates appropriate conversations, message and contacts' do
        ...
        expect(telegram_channel.inbox.conversations.count).not_to eq(0)
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.name).to eq('Sojan Jose')
        expect(Contact.where(account_id: telegram_channel.inbox.account_id).order(:id).first.additional_attributes['social_telegram_user_id']).to eq(5_171_248)
