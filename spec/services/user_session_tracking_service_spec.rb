...
RSpec.describe UserSessionTrackingService do
  ...
  describe '#create_or_update!' do
    ...
    context 'with a Starchats Mobile legacy User-Agent' do
      context 'when the UA is okhttp (Android Starchats Mobile)' do
        let(:ua) { 'okhttp/4.9.2' }

        it 'labels the session as Starchats Mobile on Android', :aggregate_failures do
          service.create_or_update!

          session = user.user_sessions.last
          expect(session.browser_name).to eq('Starchats Mobile')
      ...
      context 'when the UA is CFNetwork (iOS Starchats Mobile)' do
        let(:ua) { 'Starchats/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

        it 'labels the session as Starchats Mobile on iPhone', :aggregate_failures do
          service.create_or_update!
    ...
    context 'with X-Starchats-* structured headers' do
      let(:ua) { 'Starchats/3759 CFNetwork/3886.100.1 Darwin/27.0.0' }

      context 'when platform is ios and model is an iPhone' do
        let(:headers) do
          {
            'X-Starchats-Client-Name' => 'Starchats Mobile',
            'X-Starchats-Client-Version' => '4.7.0',
            'X-Starchats-Platform' => 'ios',
            'X-Starchats-Platform-Version' => '18.2',
            'X-Starchats-Device-Model' => 'iPhone 15 Pro'
          }
        end
        ...
        let(:headers) do
          {
            ...
            'X-Starchats-Platform' => 'ios',
            'X-Starchats-Platform-Version' => '18.2',
            'X-Starchats-Device-Model' => 'iPad Pro 11-inch'
          }
        end
        ...
        let(:headers) do
          {
            'X-Starchats-Client-Name' => 'Starchats Mobile',
            'X-Starchats-Client-Version' => '4.7.0',
            'X-Starchats-Platform' => 'android',
            'X-Starchats-Platform-Version' => '14',
            'X-Starchats-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when X-Starchats-Client-Name is blank' do
        let(:ua) { 'okhttp/4.9.2' }
        let(:headers) do
          {
            'X-Starchats-Client-Name' => '',
            'X-Starchats-Platform' => 'android',
            'X-Starchats-Device-Model' => 'Pixel 7 Pro'
          }
        end
      ...
      context 'when no X-Starchats-* headers are sent (real browser)' do
        let(:ua) { 'Mozilla/5.0 (X11; Linux x86_64; rv:124.0) Gecko/20100101 Firefox/124.0' }
        let(:headers) { {} }
