...
RSpec.describe Cosmos::AssistantStatsBuilder do
  let(:account) { create(:account) }
  let(:assistant) { create(:cosmos_assistant, account: account) }
  let(:inbox) { create(:inbox, account: account) }

  before { create(:cosmos_inbox, cosmos_assistant: assistant, inbox: inbox) }
  ...
  describe '#metrics' do
    ...
    it 'derives auto-resolution and handoff rates from reporting events on the handled set' do
      create(:reporting_event, account: account, conversation: current_convo_a,
                               name: 'conversation_cosmos_inference_resolved')
      create(:reporting_event, account: account, conversation: current_convo_b,
                               name: 'conversation_cosmos_inference_handoff')
    ...
    it 'excludes resolution events that fall outside the current window' do
      create(:reporting_event, account: account, conversation: current_convo_a,
                               name: 'conversation_cosmos_inference_resolved', created_at: 60.days.ago)
  ...
  describe '#metrics reopen_rate' do
    ...
    it 'counts a reopen that happened after the cosmos resolve' do
    ...
    it 'ignores a human resolve/reopen that happened before the cosmos resolve' do
      # Earlier resolve/reopen cycle, then Cosmos resolves later in the same window.
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
    ...
    it 'counts both inference and time-based bot resolves in the denominator' do
      ...
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
                               name: 'conversation_cosmos_inference_resolved', event_start_time: 6.days.ago, event_end_time: 6.days.ago)
      create(:reporting_event, account: account, inbox: inbox, conversation: conversation,
                               name: 'conversation_opened', value: 120, event_start_time: 6.days.ago, event_end_time: 4.days.ago)
    ...
    it 'derives the cohort from handled conversations, not current inbox membership' do
      ...
      CosmosInbox.where(cosmos_assistant: assistant).delete_all
  ...
  describe '#metrics knowledge' do
    before do
      ...
      create(:cosmos_assistant_response, assistant: assistant, account: account, status: :pending)
    ...
    it 'reports zero coverage when there are no responses' do
      Cosmos::AssistantResponse.where(assistant: assistant).delete_all
