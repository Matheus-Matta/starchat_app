...
RSpec.describe 'Api::V1::Accounts::Cosmos::AgentSessions', type: :request do
  ...
  describe 'GET /api/v1/accounts/:account_id/cosmos/agent_sessions/:id' do
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        get "/api/v1/accounts/#{account.id}/cosmos/agent_sessions/#{message.id}", as: :json
    ...
    context 'when the message has an agent session' do
      let(:document) { create(:cosmos_document, account: account, assistant: assistant) }
      let(:documented_faq) do
        create(:cosmos_assistant_response, account: account, assistant: assistant,
                                            question: 'How do I reset my password?', documentable: document)
      end
      let(:plain_faq) do
        create(:cosmos_assistant_response, account: account, assistant: assistant, question: 'How do I change my email?')
      end
      let(:pdf_document) do
        create(:cosmos_document, account: account, assistant: assistant, external_link: nil,
      ...
      let(:scenario) { create(:cosmos_scenario, account: account, assistant: assistant, title: 'Refund flow') }
      let(:run_context) do
        [
      ...
      let!(:agent_session) do
        create(:cosmos_agent_session, account: account, assistant: assistant,
      ...
      it 'returns the session with hydrated citations and scenarios' do
        get "/api/v1/accounts/#{account.id}/cosmos/agent_sessions/#{message.id}",
    ...
    context 'when the message does not belong to the account' do
      it 'returns not found' do
        ...
        get "/api/v1/accounts/#{account.id}/cosmos/agent_sessions/#{other_message.id}",
            headers: agent.create_new_auth_token, as: :json
