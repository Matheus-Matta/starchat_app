  ...
  describe 'POST /api/v1/accounts/:account_id/cosmos/bulk_actions' do
      ...
      it 'deletes the documents and returns the deleted count' do
        expect do
          post "/api/v1/accounts/#{account.id}/cosmos/bulk_actions",
               ...
               headers: admin.create_new_auth_token,
               as: :json
        end.to change(Cosmos::Document, :count).by(-2)
    ...
    context 'when syncing documents' do
        ...
        freeze_time do
          expect do
            post "/api/v1/accounts/#{account.id}/cosmos/bulk_actions",
                 ...
                 headers: admin.create_new_auth_token,
                 as: :json
          end.to have_enqueued_job(Cosmos::Documents::PerformSyncJob).on_queue('low').exactly(documents.size).times
      ...
      it 'queues documents that already have a sync in progress' do
        syncing_document = create(:cosmos_document, assistant: assistant, account: account, status: :available)
        ...
        expect do
          post "/api/v1/accounts/#{account.id}/cosmos/bulk_actions",
               ...
               headers: admin.create_new_auth_token,
               as: :json
        end.to have_enqueued_job(Cosmos::Documents::PerformSyncJob).with(syncing_document).on_queue('low')
      ...
      it 'queues stale syncing documents again' do
        ...
        freeze_time do
          syncing_document.update!(
            sync_status: :syncing,
            last_sync_attempted_at: (Cosmos::Document::SYNC_STALE_TIMEOUT + 1.minute).ago
          )
          ...
          expect do
            post "/api/v1/accounts/#{account.id}/cosmos/bulk_actions",
                 ...
                 headers: admin.create_new_auth_token,
                 as: :json
          end.to have_enqueued_job(Cosmos::Documents::PerformSyncJob).with(syncing_document)
