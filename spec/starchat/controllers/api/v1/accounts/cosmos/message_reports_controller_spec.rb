...
RSpec.describe 'Api::V1::Accounts::Cosmos::MessageReports', type: :request do
  ...
  describe 'POST /api/v1/accounts/:account_id/cosmos/message_reports' do
    let(:valid_params) do
      {
    ...
    context 'when it is an unauthenticated user' do
      it 'returns unauthorized' do
        post "/api/v1/accounts/#{account.id}/cosmos/message_reports", params: valid_params, as: :json
    ...
    context 'when on Chatwoot cloud' do
      ...
      it 'creates a message report for the reporting agent' do
        expect do
          post "/api/v1/accounts/#{account.id}/cosmos/message_reports",
               params: valid_params, headers: agent.create_new_auth_token, as: :json
        end.to change(Cosmos::MessageReport, :count).by(1)

        report = Cosmos::MessageReport.last
      ...
      it 'rejects messages that were not sent by a Cosmos assistant' do
        non_cosmos_message = create(:message, account: account, conversation: conversation)

        expect do
          post "/api/v1/accounts/#{account.id}/cosmos/message_reports",
               params: valid_params.merge(message_id: non_cosmos_message.id),
               headers: agent.create_new_auth_token, as: :json
        end.not_to change(Cosmos::MessageReport, :count)
