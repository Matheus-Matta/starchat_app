...
RSpec.describe Starchat::Api::V2::AccountsController, type: :request do
  ...
  before do
    allow(Starchat::ClearbitLookupService).to receive(:lookup).and_return(clearbit_data)
  end
  ...
  describe 'POST /api/v1/accounts' do
    ...
    it 'fetches data from clearbit and updates user and account info' do
      with_modified_env ENABLE_ACCOUNT_SIGNUP: 'true' do
        ...
        expect(Starchat::ClearbitLookupService).to have_received(:lookup).with(email)
    ...
    it 'handles errors when fetching data from clearbit' do
      with_modified_env ENABLE_ACCOUNT_SIGNUP: 'true' do
        allow(account_builder).to receive(:perform).and_return([user, account])
        allow(Starchat::ClearbitLookupService).to receive(:lookup).and_raise(StandardError)
        params = { email: email, user: nil, locale: nil, password: 'Password1!' }
