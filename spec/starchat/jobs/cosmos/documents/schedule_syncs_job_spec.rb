...
RSpec.describe Cosmos::Documents::ScheduleSyncsJob, type: :job do
  ...
  def sync_job_for(document)
    have_enqueued_job(Cosmos::Documents::PerformSyncJob).with(document)
  end

  context 'when the account has not enabled auto-sync' do
    before { account.disable_features!('cosmos_document_auto_sync') }

    it 'leaves available documents alone' do
      create(:cosmos_document, assistant: assistant, account: account, status: :available)
      clear_enqueued_jobs

      expect { described_class.new.perform }.not_to have_enqueued_job(Cosmos::Documents::PerformSyncJob)
    end
  end
  ...
  context 'when an available document has backfilled sync metadata' do
    ...
    it 'delays only the queued sync job' do
      travel_to Time.zone.local(2026, 4, 27, 10, 0, 0) do
        ...
        expect(Cosmos::Documents::PerformSyncJob)
  ...
  context 'when jitter spreads queued sync execution' do
    it 'uses a widened due window so jittered syncs do not skip the next plan run' do
      travel_to Time.zone.local(2026, 4, 27, 10, 0, 0) do
        ...
        expect { job.perform }.not_to have_enqueued_job(Cosmos::Documents::PerformSyncJob)

        document.update!(sync_status: :synced, last_synced_at: (due_window + 1.minute).ago)
        clear_enqueued_jobs

        expect { job.perform }
          .to have_enqueued_job(Cosmos::Documents::PerformSyncJob)
          .with(document)
          .on_queue('purgable')
          .at(2.hours.from_now)
      end
  ...
  context 'when more documents are due than the account cap allows' do
    ...
    it 'queues backfilled and oldest-attempted documents first' do
      newest_document = create(:cosmos_document, assistant: assistant, account: account, status: :available)
      oldest_document = create(:cosmos_document, assistant: assistant, account: account, status: :available)
      backfilled_document = create(:cosmos_document, assistant: assistant, account: account, status: :available)
      ...
      expect(Cosmos::Documents::PerformSyncJob).not_to have_been_enqueued.with(newest_document)
    end
  end
  ...
  context 'when sync caps are configured' do
    it 'uses installation config caps for per-account and global limits' do
      ...
      second_assistant = create(:cosmos_assistant, account: second_account)

      first_account_documents = create_list(:cosmos_document, 3, assistant: assistant, account: account, status: :available)
      second_account_documents = create_list(:cosmos_document, 3, assistant: second_assistant, account: second_account, status: :available)
      ...
      expect { described_class.new.perform }
        .to have_enqueued_job(Cosmos::Documents::PerformSyncJob).exactly(3).times
    end
  end
  ...
  context 'when a document has been queued longer than the worker lock timeout' do
    it 'leaves it alone so queue lag is not treated as a dead worker' do
      ...
      document.update!(
        sync_status: :syncing,
        last_sync_attempted_at: (Cosmos::Documents::PerformSyncJob::LOCK_TIMEOUT + 1.minute).ago
      )
      clear_enqueued_jobs
  ...
  context 'when the only eligible document is a PDF' do
    it 'leaves it alone since PDFs are not syncable' do
      pdf_document = build(:cosmos_document, assistant: assistant, account: account, status: :available)
