...
RSpec.describe Cosmos::InboxPendingConversationsResolutionJob, type: :job do
  ...
  it 'adds the correct activity message after resolution by Cosmos' do
    ...
    it 'skips auto-action if conversation receives new activity after evaluation' do
      mock_service = instance_double(Cosmos::ConversationCompletionService)
      ...
      allow(Cosmos::ConversationCompletionService).to receive(:new).and_return(mock_service)
    ...
    it 'falls back to legacy time-based resolve when legacy auto-resolve is forced' do
      inbox.account.update!(cosmos_auto_resolve_mode: 'legacy')
      allow(Cosmos::ConversationCompletionService).to receive(:new)
  ...
  context 'when LLM evaluation returns complete' do
    before do
      ...
      allow(inbox.account).to receive(:feature_enabled?).with('cosmos_tasks').and_return(true)
      mock_service = instance_double(Cosmos::ConversationCompletionService)
    ...
    it 'adds the correct activity message after resolution' do
      ...
      expected_content = I18n.t(
        'conversations.activity.cosmos.resolved_with_reason',
        user_name: cosmos_assistant.name,
    ...
    it 'creates a cosmos inference resolved reporting event' do
  ...
  context 'when LLM evaluation returns incomplete' do
    ...
    it 'creates handoff message with configured content' do
      ...
      cosmos_assistant.update!(config: { 'handoff_message' => handoff_message })
      inbox.reload
      allow(inbox.account).to receive(:feature_enabled?).and_call_original
      allow(inbox.account).to receive(:feature_enabled?).with('cosmos_tasks').and_return(true)
    ...
    it 'adds the correct activity message after handoff' do
      ...
      expected_content = I18n.t(
        'conversations.activity.cosmos.open_with_reason',
        user_name: cosmos_assistant.name,
    ...
    it 'creates a cosmos inference handoff reporting event' do
      perform_enqueued_jobs do
        described_class.perform_now(inbox)
      end
  ...
  it 'does not resolve conversations when auto-resolve is disabled at execution time' do
    inbox.account.update!(cosmos_auto_resolve_mode: 'disabled')
  ...
  it 'falls back to disabled mode from legacy settings key' do
    inbox.account.update!(settings: inbox.account.settings.merge('cosmos_disable_auto_resolve' => true))
