...
RSpec.describe Cosmos::Llm::ConversationFaqJob, type: :job do
  let(:account) { create(:account) }
  let(:inbox) { create(:inbox, account: account) }
  let(:assistant) { create(:cosmos_assistant, account: account, config: { feature_faq: true }) }
  let(:conversation) { create(:conversation, account: account, inbox: inbox, first_reply_created_at: Time.zone.now) }
  let(:faq_service) { instance_double(Cosmos::Llm::ConversationFaqService, generate_suggestions: []) }
  let(:lock_manager) { instance_double(Redis::LockManager, lock: true, unlock: true) }
  let(:lock_key) { "COSMOS_CONVERSATION_FAQ_LOCK::#{assistant.id}::en" }

  before do
    create(:cosmos_inbox, inbox: inbox, cosmos_assistant: assistant)
    conversation.update!(status: :resolved)
    allow(Redis::LockManager).to receive(:new).and_return(lock_manager)
    allow(Cosmos::Llm::ConversationFaqService).to receive(:new).and_return(faq_service)
  end

  describe '#perform' do
    it 'uses the assistant captured when the job was enqueued' do
      replacement_assistant = create(:cosmos_assistant, account: account, config: { feature_faq: true })
      inbox.cosmos_inbox.update!(cosmos_assistant: replacement_assistant)

      expect(inbox.reload.cosmos_assistant).to eq(replacement_assistant)
      expect(Cosmos::Llm::ConversationFaqService).to receive(:new)
    ...
    it 'locks FAQ grouping for the assistant and normalized language' do
      ...
      expected_key = "COSMOS_CONVERSATION_FAQ_LOCK::#{assistant.id}::pt"
