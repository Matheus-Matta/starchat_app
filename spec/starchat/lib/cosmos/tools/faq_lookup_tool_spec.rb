...
RSpec.describe Cosmos::Tools::FaqLookupTool, type: :model do
  ...
  before do
    ...
    embedding_service = instance_double(Cosmos::Llm::EmbeddingService)
  ...
  describe '#perform' do
    context 'when FAQs exist' do
      ...
      let!(:response1) do
        create(:cosmos_assistant_response,
               assistant: assistant,
      ...
      before do
        ...
        allow(Cosmos::AssistantResponse).to receive(:nearest_neighbors).and_return(
          Cosmos::AssistantResponse.where(id: [response1.id, response2.id])
        )
      end
      ...
      it 'records retrieved faq ids and document ids into Starchats metadata' do
        tool.perform(tool_context, query: 'password reset')
    ...
    context 'when no FAQs found' do
      before do
        ...
        allow(Cosmos::AssistantResponse).to receive(:nearest_neighbors).and_return(Cosmos::AssistantResponse.none)
      end
