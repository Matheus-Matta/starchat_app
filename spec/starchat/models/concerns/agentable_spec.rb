...
RSpec.describe Concerns::Agentable do
  ...
  before do
    InstallationConfig.where(name: 'COSMOS_OPEN_AI_MODEL').destroy_all
    allow(Agents::Agent).to receive(:new).and_return(mock_agents_agent)
    allow(Cosmos::PromptRenderer).to receive(:render).and_return('rendered_template')
  end
  ...
  describe '#agent' do
    it 'creates an Agents::Agent with correct parameters' do
      expect(Agents::Agent).to receive(:new).with(
        ...
        temperature: 0.8,
        response_schema: Cosmos::ResponseSchema
      )
  ...
  describe '#agent_instructions' do
    it 'calls Cosmos::PromptRenderer with base context' do
  ...
  describe '#agent_model' do
    ...
    it 'returns the Cosmos V2 default when Cosmos V2 is enabled' do
      create(:installation_config, name: 'COSMOS_OPEN_AI_MODEL', value: 'gpt-4.1-nano')
  ...
  describe '#agent_response_schema' do
    it 'returns Cosmos::ResponseSchema' do
      expect(dummy_instance.send(:agent_response_schema)).to eq(Cosmos::ResponseSchema)
    end
  end
