...
RSpec.describe Cosmos::CustomTool, type: :model do
  ...
  describe 'validations' do
    ...
    describe 'slug uniqueness' do
      ...
      it 'validates uniqueness of slug scoped to account' do
        create(:cosmos_custom_tool, account: account, slug: 'custom_test_tool')
        duplicate = build(:cosmos_custom_tool, account: account, slug: 'custom_test_tool')
      ...
      it 'allows same slug across different accounts' do
        ...
        create(:cosmos_custom_tool, account: account, slug: 'custom_test_tool')
        different_account_tool = build(:cosmos_custom_tool, account: account2, slug: 'custom_test_tool')
    ...
    describe 'param_schema validation' do
      ...
      it 'is valid with proper param_schema' do
        tool = build(:cosmos_custom_tool, account: account, param_schema: [
  ...
  describe 'scopes' do
    ...
    describe '.enabled' do
      it 'returns only enabled custom tools' do
        enabled_tool = create(:cosmos_custom_tool, account: account, enabled: true)
        disabled_tool = create(:cosmos_custom_tool, account: account, enabled: false)
  ...
  describe 'slug generation' do
    ...
    it 'generates slug from title on creation' do
      tool = create(:cosmos_custom_tool, account: account, title: 'Fetch Order Status')

      expect(tool.slug).to eq('custom_fetch_order_status')
    end

    it 'adds custom_ prefix to generated slug' do
      tool = create(:cosmos_custom_tool, account: account, title: 'My Tool')

      expect(tool.slug).to start_with('custom_')
    end

    it 'does not override manually set slug' do
      tool = create(:cosmos_custom_tool, account: account, title: 'Test Tool', slug: 'custom_manual_slug')

      expect(tool.slug).to eq('custom_manual_slug')
    end

    it 'handles slug collisions by appending random suffix' do
      create(:cosmos_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool')
      tool2 = create(:cosmos_custom_tool, account: account, title: 'Test Tool')
    ...
    it 'handles multiple slug collisions' do
      create(:cosmos_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool')
      create(:cosmos_custom_tool, account: account, title: 'Test Tool', slug: 'custom_test_tool_abc123')
      tool3 = create(:cosmos_custom_tool, account: account, title: 'Test Tool')
    ...
    it 'does not generate slug when title is blank' do
      tool = build(:cosmos_custom_tool, account: account, title: nil)
    ...
    it 'parameterizes title correctly' do
      tool = create(:cosmos_custom_tool, account: account, title: 'Fetch Order Status & Details!')
  ...
  describe 'factory' do
    it 'creates a valid custom tool with default attributes' do
      tool = create(:cosmos_custom_tool)
    ...
    it 'creates valid tool with POST trait' do
      tool = create(:cosmos_custom_tool, :with_post)
    ...
    it 'creates valid tool with bearer auth trait' do
      tool = create(:cosmos_custom_tool, :with_bearer_auth)
    ...
    it 'creates valid tool with basic auth trait' do
      tool = create(:cosmos_custom_tool, :with_basic_auth)
    ...
    it 'creates valid tool with api key trait' do
      tool = create(:cosmos_custom_tool, :with_api_key)
  ...
  describe 'Toolable concern' do
    ...
    describe '#build_request_url' do
      it 'returns static URL when no template variables present' do
        tool = create(:cosmos_custom_tool, account: account, endpoint_url: 'https://api.example.com/orders')

        expect(tool.build_request_url({})).to eq('https://api.example.com/orders')
      end

      it 'renders URL template with params' do
        tool = create(:cosmos_custom_tool, account: account, endpoint_url: 'https://api.example.com/orders/{{ order_id }}')
    ...
    describe '#build_request_body' do
      it 'returns nil when request_template is blank' do
        tool = create(:cosmos_custom_tool, account: account, request_template: nil)
    ...
    describe '#build_auth_headers' do
      it 'returns empty hash for none auth type' do
        tool = create(:cosmos_custom_tool, account: account, auth_type: 'none')

        expect(tool.build_auth_headers).to eq({})
      end

      it 'returns bearer token header' do
        tool = create(:cosmos_custom_tool, :with_bearer_auth, account: account)

        expect(tool.build_auth_headers).to eq({ 'Authorization' => 'Bearer test_bearer_token_123' })
      end

      it 'returns API key header' do
        tool = create(:cosmos_custom_tool, :with_api_key, account: account)
      ...
      it 'returns empty hash for basic auth' do
        tool = create(:cosmos_custom_tool, :with_basic_auth, account: account)
    ...
    describe '#format_response' do
      it 'returns raw response when no response_template' do
        tool = create(:cosmos_custom_tool, account: account, response_template: nil)
    ...
    describe '#build_metadata_headers' do
      ...
      it 'includes account and assistant metadata' do
        ...
        expect(headers['X-Starchats-Account-Id']).to eq(account.id.to_s)
        expect(headers['X-Starchats-Assistant-Id']).to eq('123')
      end

      it 'includes tool slug' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Starchats-Tool-Slug']).to eq('custom_test_tool')
      end

      it 'includes conversation metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Starchats-Conversation-Id']).to eq(conversation.id.to_s)
        expect(headers['X-Starchats-Conversation-Display-Id']).to eq(conversation.display_id.to_s)
      end

      it 'includes contact metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Starchats-Contact-Id']).to eq(contact.id.to_s)
        expect(headers['X-Starchats-Contact-Email']).to eq(contact.email)
      end

      it 'includes contact inbox verification metadata when present' do
        headers = tool.build_metadata_headers(state)

        expect(headers['X-Starchats-Contact-Inbox-Id']).to eq(conversation.contact_inbox.id.to_s)
        expect(headers['X-Starchats-Contact-Inbox-Verified']).to eq(conversation.contact_inbox.hmac_verified.to_s)
      end
      ...
      it 'handles missing conversation gracefully' do
        ...
        expect(headers['X-Starchats-Conversation-Id']).to be_nil
        expect(headers['X-Starchats-Conversation-Display-Id']).to be_nil
        expect(headers['X-Starchats-Account-Id']).to eq(account.id.to_s)
      end
      ...
      it 'handles missing contact gracefully' do
        ...
        expect(headers['X-Starchats-Contact-Id']).to be_nil
        expect(headers['X-Starchats-Contact-Email']).to be_nil
        expect(headers['X-Starchats-Account-Id']).to eq(account.id.to_s)
      end
      ...
      it 'handles missing contact inbox gracefully' do
        ...
        expect(headers['X-Starchats-Contact-Inbox-Id']).to be_nil
        expect(headers['X-Starchats-Contact-Inbox-Verified']).to eq('false')
      end
      ...
      it 'omits contact email header when email is blank' do
        ...
        expect(headers).not_to have_key('X-Starchats-Contact-Email')
      end
      ...
      it 'omits contact phone header when phone number is blank' do
        ...
        expect(headers).not_to have_key('X-Starchats-Contact-Phone')
      end
    ...
    describe '#tool' do
      ...
      it 'sets description on the tool class' do
        tool = create(:cosmos_custom_tool, account: account, description: 'Fetches order data')
      ...
      it 'sets parameters on the tool class' do
        tool = create(:cosmos_custom_tool, :with_params, account: account)
      ...
      it 'works with empty param_schema' do
        tool = create(:cosmos_custom_tool, account: account, param_schema: [])
