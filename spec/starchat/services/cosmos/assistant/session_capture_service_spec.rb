...
RSpec.describe Cosmos::Assistant::SessionCaptureService do
  ...
  describe '#capture' do
    it 'creates the session' do
      expect { service.capture }.to change(Cosmos::AgentSession, :count).by(1)
    end
    ...
    it 'does nothing when there is no run result' do
      ...
      expect { service.capture }.not_to change(Cosmos::AgentSession, :count)
    end
  ...
  describe '#capture!' do
    ...
    it 'extracts every scenario that authored a message in the current turn' do
      first_scenario = create(:cosmos_scenario, assistant: assistant, account: account)
      second_scenario = create(:cosmos_scenario, assistant: assistant, account: account)
    ...
    it 'does not capture a scenario belonging to another assistant' do
      scenario = create(:cosmos_scenario, assistant: create(:cosmos_assistant, account: account), account: account)
      run_context[:conversation_history] = [
