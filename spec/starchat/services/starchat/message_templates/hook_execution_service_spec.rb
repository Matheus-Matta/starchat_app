...
RSpec.describe MessageTemplates::HookExecutionService do
  ...
  context 'when cosmos assistant is configured' do
    context 'when within business hours' do
      ...
      it 'schedules cosmos response job for incoming messages on pending conversations' do
        expect(Cosmos::Conversation::ResponseBuilderJob).to receive(:perform_later).with(conversation, assistant)
    ...
    context 'when outside business hours' do
      ...
      it 'schedules cosmos response job outside business hours (Cosmos always responds when configured)' do
        expect(Cosmos::Conversation::ResponseBuilderJob).to receive(:perform_later).with(conversation, assistant)

        create(:message, conversation: conversation, message_type: :incoming)
      end

      it 'performs cosmos handoff when quota is exceeded (OOO template will kick in after handoff)' do
        account.update!(
          limits: { 'cosmos_responses' => 100 },
          custom_attributes: account.custom_attributes.merge('cosmos_responses_usage' => 100)
        )
      ...
      it 'does not send out of office message when Cosmos is handling' do
    ...
    context 'when business hours are not enabled' do
      ...
      it 'schedules cosmos response job regardless of time' do
        expect(Cosmos::Conversation::ResponseBuilderJob).to receive(:perform_later).with(conversation, assistant)
    ...
    context 'when cosmos quota is exceeded within business hours' do
      before do
        inbox.update!(working_hours_enabled: true)
        inbox.working_hours.find_by(day_of_week: Time.current.in_time_zone(inbox.timezone).wday).update!(
          open_all_day: true,
  ...
  context 'when no cosmos assistant is configured' do
    before do
      CosmosInbox.where(inbox: inbox).destroy_all
    end

    it 'does not schedule cosmos response job' do
      expect(Cosmos::Conversation::ResponseBuilderJob).not_to receive(:perform_later)
  ...
  context 'when greeting and out of office messages with Cosmos enabled' do
    context 'when conversation is pending (Cosmos is handling)' do
      before do
  ...
  context 'when Cosmos is not configured' do
    before do
      CosmosInbox.where(inbox: inbox).destroy_all
    end
  ...
  context 'when Cosmos quota is exceeded and handoff happens' do
    before do
      account.update!(
        limits: { 'cosmos_responses' => 100 },
        custom_attributes: account.custom_attributes.merge('cosmos_responses_usage' => 100)
