# Lists the underlying records behind a single Cosmos assistant stat card, so a
# viewer can drill from an aggregate (e.g. "auto-resolution 42%") into the exact
# conversations that produced it.
#
# The window is resolved by Cosmos::AssistantStatsWindow from the same `range`
...
class Cosmos::AssistantDrilldownBuilder
  ASSISTANT_SENDER_TYPE = 'Cosmos::Assistant'.freeze
  RESOLVED_EVENT_NAMES = Cosmos::AssistantStatsBuilder::RESOLVED_EVENT_NAMES
  HANDOFF_EVENT_NAMES = Cosmos::AssistantStatsBuilder::HANDOFF_EVENT_NAMES
  ...
  # Cosmos resolves in the window, excluding bot-resolved rows whose conversation
  ...
  def resolved_events
    ...
    account.reporting_events
           ...
           .where("NOT (name = ? AND conversation_id IN (#{handoff_ids.to_sql}))",
                  Cosmos::AssistantStatsBuilder::BOT_RESOLVED_EVENT_NAME)
  end

  # Auto-resolved conversations that reopened at/after their Cosmos resolve,
