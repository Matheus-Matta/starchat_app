# Computes per-assistant overview metrics for the Cosmos Overview page.
...
class Cosmos::AssistantStatsBuilder
  RESOLVED_EVENT_NAMES = %w[conversation_cosmos_inference_resolved conversation_bot_resolved].freeze
  HANDOFF_EVENT_NAMES = %w[conversation_cosmos_inference_handoff conversation_bot_handoff].freeze
  ...
  def initialize(assistant, range = Cosmos::AssistantStatsWindow::DEFAULT_RANGE, timezone_offset = nil)
    @assistant = assistant
    @account = assistant.account
    @window = Cosmos::AssistantStatsWindow.new(range, timezone_offset)
  end
  ...
  def handled_scope(range)
    account.messages.where(sender_type: 'Cosmos::Assistant', sender_id: assistant.id, created_at: range)
  end
  ...
  # Of the conversations Cosmos auto-resolved, the share reopened afterwards. The cohort is
  ...
  def reopen_rate(range)
    ...
    # Cosmos resolves and keep only reopens at/after one of them, so a human resolve/reopen earlier
    # in the same window isn't mistaken for a reopen-after-Cosmos-resolve. (Comparing the reopen's
  ...
  def knowledge
    approved, pending, documents = Cosmos::AssistantResponse.by_assistant(assistant.id).reorder(nil).pick(
      Arel.sql("COUNT(*) FILTER (WHERE status = #{Cosmos::AssistantResponse.statuses['approved']})"),
      Arel.sql("COUNT(*) FILTER (WHERE status = #{Cosmos::AssistantResponse.statuses['pending']})"),
      Arel.sql("(SELECT COUNT(*) FROM cosmos_documents WHERE assistant_id = #{assistant.id.to_i})")
    )
