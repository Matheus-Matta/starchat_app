# Resolves the current and previous comparison windows for Cosmos assistant
...
#
# Shared by Cosmos::AssistantStatsBuilder (which needs both windows) and
# Cosmos::AssistantDrilldownBuilder (which drills into the current window), so a
# drilldown always covers exactly the rows its stat card counted.
class Cosmos::AssistantStatsWindow
