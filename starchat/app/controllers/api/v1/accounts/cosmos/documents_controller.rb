class Api::V1::Accounts::Cosmos::DocumentsController < Api::V1::Accounts::BaseController
  ...
  def sync
    ...
    Cosmos::Documents::PerformSyncJob.perform_later(@document)
    head :accepted
  end
  ...
  def set_documents
    @documents = Current.account.cosmos_documents.with_attached_pdf_file.includes(:assistant)
  end
  ...
  def with_responses_count(scope)
    scope.left_joins(:responses)
         .select('cosmos_documents.*, COUNT(cosmos_assistant_responses.id) AS responses_count')
         .group('cosmos_documents.id')
  end
  ...
  def apply_search(scope, search_key)
    ...
    scope.where('cosmos_documents.name ILIKE :query OR cosmos_documents.external_link ILIKE :query', query: query)
  end
  ...
  def stale_documents(scope)
    ...
    scope.sync_synced.where(Cosmos::Document.arel_table[:last_synced_at].lt(current_sync_interval.ago))
  end
  ...
  def up_to_date_documents(scope)
    ...
    documents.where(Cosmos::Document.arel_table[:last_synced_at].gteq(current_sync_interval.ago))
  end

  def current_sync_interval
    return @current_sync_interval if defined?(@current_sync_interval)

    @current_sync_interval = Current.account.cosmos_document_sync_interval
  end
