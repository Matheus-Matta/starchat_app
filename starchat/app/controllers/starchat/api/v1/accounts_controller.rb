class Starchat::Api::V1::AccountsController < Api::BaseController
  ...
  def select_billing_currency
    ...
    currency = Starchat::Billing::Currencies.normalize(params[:currency])
    return render_could_not_create_error(I18n.t('errors.billing.invalid_currency')) unless Starchat::Billing::Currencies.supported?(currency)
  ...
  def ensure_stripe_customer
    ...
    Starchat::CreateStripeCustomerJob.perform_later(@account)
  end

  def currency_selection_payload
    {
      currency_selection_required: true,
      currency_options: Starchat::Billing::Currencies::SUPPORTED,
      suggested_currency: Starchat::Billing::Currencies.for_locale(@account.locale)
    }
  end
  ...
  def create_stripe_billing_session(customer_id)
    session = Starchat::Billing::CreateSessionService.new.create_session(customer_id)
    render_redirect_url(session.url)
  end

  def cancel_cloud_subscriptions_for_deletion
    Starchat::Billing::CancelCloudSubscriptionsService.new(account: @account).perform
