...
class CosmosModelOverridesField < Administrate::Field::Base
  def feature_rows
  ...
  def selected_override(feature_key)
    resource.cosmos_models&.[](feature_key).presence
  end

  def default_model_id(feature_key)
    return Llm::FeatureRouter::COSMOS_V2_ASSISTANT_MODEL if feature_key == 'assistant' && resource.feature_enabled?('cosmos_integration_v2')
  ...
  def feature_name(feature_key)
    I18n.t("super_admin.cosmos_model_overrides.features.#{feature_key}", default: feature_key.humanize)
  end

  def source_label(source)
    I18n.t("super_admin.cosmos_model_overrides.sources.#{source}")
  end
end
