module Cosmos::Conversation::V1FalsePromiseHandler
  ...
  def detect_v1_false_promise(message_history)
    detection = Cosmos::Llm::AssistantFalsePromiseService.new(
      assistant: @assistant,
  ...
  def v1_false_promise_harness_enabled?
    ActiveModel::Type::Boolean.new.cast(account.cosmos_false_promise_harness_enabled)
  end

  def log_v1_false_promise_detection(detection)
    Rails.logger.info(
      "[COSMOS][ResponseBuilderJob] V1 false promise harness account=#{account.id} " \
