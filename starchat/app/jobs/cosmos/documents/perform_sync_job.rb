class Cosmos::Documents::PerformSyncJob < MutexApplicationJob
  queue_as :low
  ...
  retry_on StandardError, wait: 5.seconds, attempts: 3 do |job, error|
    ...
    StarchatsExceptionTracker.new(error, account: document.account).capture_exception
  ...
  discard_on(Cosmos::Documents::SyncService::PermanentSyncError)
  ...
  retry_on(
    Cosmos::Documents::SyncService::TransientSyncError,
  ...
  rescue Cosmos::Documents::SyncService::PermanentSyncError => e
    log_failure_and_raise(document, :permanent_failure, e, start_time)
  rescue Cosmos::Documents::SyncService::TransientSyncError => e
  ...
  def perform_sync(document, start_time)
    ...
    result = Cosmos::Documents::SyncService.new(document.reload).perform
  ...
  def log_sync_outcome(document, **fields)
    ...
    Rails.logger.info("[Cosmos::Documents::PerformSyncJob] #{payload.to_json}")
  end
