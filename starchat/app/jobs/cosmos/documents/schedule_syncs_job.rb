class Cosmos::Documents::ScheduleSyncsJob < ApplicationJob
  queue_as :scheduled_jobs

  DEFAULT_PER_ACCOUNT_BATCH_LIMIT = 50
  DEFAULT_GLOBAL_BATCH_LIMIT = 1000
  SYNC_STALE_TIMEOUT = Cosmos::Document::SYNC_STALE_TIMEOUT
  ...
  def process_due_document(document, interval, result)
    ...
    Cosmos::Documents::PerformSyncJob.set(queue: :purgable, wait: sync_execution_delay).perform_later(document)
    @remaining_global_capacity -= 1
    result[:enqueued] += 1
  end

  def due_documents(account, interval)
    syncing = Cosmos::Document.sync_statuses[:syncing]
    ...
    documents = account.cosmos_documents.syncable.where(status: :available).where(
  ...
  def log_scheduler_summary(stats)
    ...
    Rails.logger.info("[Cosmos::Documents::ScheduleSyncsJob] #{payload.to_json}")
  end
end
