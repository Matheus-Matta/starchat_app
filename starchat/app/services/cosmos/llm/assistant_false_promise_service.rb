class Cosmos::Llm::AssistantFalsePromiseService < Llm::BaseAiService
  ...
  def detect(message_history:, assistant_response:)
    ...
    response = instrument_llm_call(instrumentation_params(user_prompt)) do
      chat(model: @model, temperature: @temperature)
        .with_schema(Cosmos::AssistantFalsePromiseSchema)
    ...
    Rails.logger.warn(
      "[COSMOS][AssistantFalsePromise] Failed for conversation #{@conversation.display_id}: #{e.class.name}: #{e.message}"
    )
  ...
  def normalize_response(parsed, raw_content)
    ...
    return invalid_response(raw_content) unless Cosmos::AssistantFalsePromiseSchema::DECISIONS.include?(decision)
  ...
  def instrumentation_params(user_prompt)
    {
      span_name: 'llm.cosmos.assistant_false_promise_detector',
      model: @model,
  ...
  def system_prompt
    Cosmos::Llm::SystemPromptsService.assistant_false_promise_detector
  end
end
