class Cosmos::Llm::PaginatedFaqGeneratorService < Llm::BaseOpenAiService
  ...
  def process_page_chunk(start_page, end_page)
    ...
    Rails.logger.error I18n.t('cosmos.documents.page_processing_error', start: start_page, end: end_page, error: e.message)
  ...
  def page_chunk_prompt(start_page, end_page)
    Cosmos::Llm::SystemPromptsService.paginated_faq_generator(start_page, end_page, @language)
  end
  ...
  def standard_chat_parameters
    {
      ...
      messages: [
        {
          role: 'system',
          content: Cosmos::Llm::SystemPromptsService.faq_generator(@language)
        },
        {
          role: 'user',
          content: @content
