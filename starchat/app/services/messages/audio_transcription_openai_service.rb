# Cloud transcription engine (OpenAI Whisper API). Paid — consumes the account's
# Cosmos AI response credits, gated by the `cosmos_integration` feature flag.
# Selected via account.audio_transcription_provider == 'openai'.
class Messages::AudioTranscriptionOpenaiService < Llm::BaseOpenAiService
  ...
  # OpenAI expects a bare ISO-639-1 code ("pt", "en"); account.locale can be
  # region-qualified ("pt_BR"). Omitting the param (nil, stripped by .compact
  # above) lets OpenAI auto-detect when the account has no locale set.
