...
module Starchat::Billing::PlanConfiguration
  ...
  def price_ids_by_currency(plan)
    ...
    case raw
    when Hash then raw.transform_keys { |key| Starchat::Billing::Currencies.normalize(key) }
    when Array then { Starchat::Billing::Currencies::DEFAULT => raw }
    else {}
    end
  end
  ...
  def price_id_for(plan, currency)
    ...
    code = Starchat::Billing::Currencies.to_supported(currency)

    (by_currency[code].presence ||
     by_currency[Starchat::Billing::Currencies::DEFAULT].presence ||
