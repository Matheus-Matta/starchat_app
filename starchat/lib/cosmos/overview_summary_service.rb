# Generates the LLM welcome summary for the Cosmos Overview page from the
# assistant's stats hash (see Cosmos::AssistantStatsBuilder). Renders the
# cosmos_overview_summary.liquid prompt and returns markdown.
class Cosmos::OverviewSummaryService < Cosmos::BaseTaskService
  ...
  def system_prompt
    Liquid::Template.parse(prompt_from_file('cosmos_overview_summary')).render(prompt_variables)
  end
  ...
  # response, so it should not consume or be blocked by the cosmos_responses quota.
